...
PG_ADMIN="pgadmin"
# Reuse ACR from audience-insights (already provisioned, Standard tier free for 31 days)
ACR_NAME="audienceinsightsacr"
...
ok "Azure CLI — $(az account show --query name -o tsv)"
...
if az vm show --name "$VM_NAME" --resource-group "$RG" &>/dev/null; then
    ...
    for LOC in $LOCATION $FALLBACK_LOCATIONS; do
        for SIZE in "Standard_B1s" "Standard_B2ats_v2" "Standard_B2pts_v2"; do
            ...
            warn "$SIZE in $LOC failed"
        done
    done
    $CREATED || fail "No VM SKU available in any region"
fi

az vm open-port --resource-group "$RG" --name "$VM_NAME" --port 22  --priority 1000 --output none 2>/dev/null || true
az vm open-port --resource-group "$RG" --name "$VM_NAME" --port 80  --priority 1001 --output none 2>/dev/null || true
az vm open-port --resource-group "$RG" --name "$VM_NAME" --port 443 --priority 1002 --output none 2>/dev/null || true
...
for PIP_NAME in "${VM_NAME}PublicIP" "${VM_NAME}-pip" "$(az network public-ip list --resource-group "$RG" --query "[0].name" -o tsv 2>/dev/null)"; do
    if [[ -n "$PIP_NAME" ]]; then
        VM_IP=$(az network public-ip show --resource-group "$RG" --name "$PIP_NAME" --query ipAddress -o tsv 2>/dev/null)
        [[ -n "$VM_IP" ]] && break
    fi
done
[[ -z "$VM_IP" ]] && fail "Could not get VM public IP"
...
if az postgres flexible-server show --name "$PG_SERVER" --resource-group "$RG" &>/dev/null; then
    ...
    for LOC in $LOCATION $FALLBACK_LOCATIONS; do
        ...
        warn "PostgreSQL creation failed in $LOC, trying next..."
    done
    $CREATED || fail "Could not create PostgreSQL in any region"
...
ok "PostgreSQL firewall: VM IP allowed"
...
# ─── ACR login on VM ───
sep
ACR_LOGIN=$(az acr show --name "$ACR_NAME" --resource-group "audience-insights-rg" --query "loginServer" -o tsv 2>/dev/null || echo "$ACR_NAME.azurecr.io")
warn "ACR: $ACR_LOGIN (reusing from audience-insights)"
...
echo "  📦 ACR:          $ACR_LOGIN (shared with audience-insights)"
echo ""
