...
| 🟡 4 | **PostgreSQL RLS scope incomplete** | `observations` now has an opt-in database policy. Other tenant-scoped tables still rely on application filters. | **Partially resolved.** `RLS_ENABLED=true` enables a reversible observations policy using the `app.tenant_id` session setting; global rows remain visible. PostgreSQL integration coverage proves a tenant cannot read another tenant through an unfiltered query. Extend table-by-table only after each access pattern is verified. | Phase 5 |
...
| ✅ 6 | **Auth coverage incomplete** | The formerly unprotected routers included analytics, reporting, subscriptions, notifications, insights, vector search, ETL, background processing, and API-key management. Mongo/scraper also depended on an unavailable Mongo deployment. | **Resolved.** Default v1 router wiring applies JWT authentication and role guards by router, plus a regression test that rejects any unprotected v1 route outside the explicit auth bootstrap endpoints. Production v1 uses a tenant/subject token bucket; the older fixed-window and session/bearer/JWT comparison endpoints are opt-in (`AUTH_DEMO_ROUTES_ENABLED=true`). Mongo and scraper remain deliberately unmounted unless Mongo is explicitly enabled and available. | Phase 4 |
