...
| 🟠 7 | **No dedicated agent router tests** | Agent endpoints (`/enrich`, `/enrich/review`, `/resume`, `/stream`) have zero dedicated tests. | **Deferred to post-MVP.** Agent enrichment (LangGraph StateGraph, HITL review) is a post-MVP feature per roadmap. | Cross-cut |
