...
| Authentication, authorization, tenancy, RLS | **Core** | [security architecture](security-architecture.md), [`security/`](../../services/ingestor/core/security/), [observation RLS tests](../../services/ingestor/tests/integration/test_observations_rls.py), [incident RLS tests](../../services/ingestor/tests/integration/test_dependency_incidents_rls.py) | Extend RLS table-by-table after each access path is verified |
...
| WebSocket / real-time push | **Lab** | [`routers/ws.py`](../../services/ingestor/api/routes/ws.py), [WebSocket tests](../../services/ingestor/tests/integration/observations/test_ws.py) | Adopt only when a real-time dashboard client requires live event streaming |
