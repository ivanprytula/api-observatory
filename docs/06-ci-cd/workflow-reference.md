...
design, see CI/CD + IaC + GitOps Portable Strategy.
...
Unit and integration tests: see `test`, `test-int`, and `test-cov` recipes in `Justfile`.
...
- CI/CD + IaC + GitOps Portable Strategy — deployment strategy, IaC, GitOps design
- Prebuilt CI Image — prebuilt CI image lifecycle
- GitHub Actions Security Hardening — SHA pinning for action refs
