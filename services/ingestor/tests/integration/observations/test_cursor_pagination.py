...
async def test_cursor_pagination_basic_traversal(
    client: AsyncClient, created_observations: list[dict], limit: int
) -> None:
    """Verify cursor pagination can traverse all observations in multiple pages."""
    # Arrange: create 10+ observations
    _ = {r["id"] for r in created_observations}

    # Act: fetch observations using cursor pagination
    seen_ids = set()
    cursor = None
    page_count = 0
    ...
    while page_count < max_pages:
        ...
        page_ids = {r["id"] for r in observations}
    ...
    # Verify we collected some observations
    assert len(seen_ids) > 0, "cursor pagination returned no observations"


async def test_cursor_pagination_has_more_flag(
    client: AsyncClient, created_observations: list[dict]
) -> None:
    """Verify has_more flag correctly indicates if more observations exist."""
    ...
    # Act & Assert
    assert response.status_code == 200
    if len(created_observations) > 1:
        # If multiple observations exist, first page should have has_more=True
        assert body["has_more"] is True
        assert body["next_cursor"] is not None
    else:
        # If 1 or 0 observations, has_more should be False
...
async def test_cursor_pagination_with_source_filter(
    ...
    # Arrange: create observations with different sources
    ...
    # Assert: all observations should be source-A
    assert response.status_code == 200
    for observation in body["observations"]:
        assert observation["source"] == "source-A"
...
async def test_cursor_pagination_stable_under_inserts(
    ...
    """Verify cursor pagination is stable when new observations are inserted concurrently.

    This test demonstrates the advantage of cursor pagination:
    offset/limit pagination would skip or duplicate observations when inserts happen.
    """
    ...
    body1 = response.json()
    first_page_ids = {r["id"] for r in body1["observations"]}
    ...
    if next_cursor:
        ...
        body2 = response.json()
        second_page_ids = {r["id"] for r in body2["observations"]}

        # Assert: no overlap between pages (cursor-pagination is stable)
        assert len(first_page_ids & second_page_ids) == 0, (
            "Cursor pagination should not have overlapping observations between pages"
        )
