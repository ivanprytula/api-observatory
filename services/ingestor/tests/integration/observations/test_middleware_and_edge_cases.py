...
class TestPaginationEdgeCases:
    ...
    async def test_list_with_zero_offset(self, client: AsyncClient) -> None:
        ...
        await client.post("/api/v1/observations", json=OBSERVATION_API)
        await client.post("/api/v1/observations", json=OBSERVATION_API)

        r = await client.get("/api/v1/observations?offset=0&limit=10")
        assert r.status_code == 200
        assert len(r.json()) >= 2

    async def test_list_with_offset_beyond_count(self, client: AsyncClient) -> None:
        """Offset beyond observation count returns empty list."""
        r = await client.get("/api/v1/observations?offset=1000&limit=10")
        assert r.status_code == 200
        body = r.json()
        # Response should have 'observations' and 'pagination' keys
    ...
    async def test_list_with_negative_limit(self, client: AsyncClient) -> None:
        ...
        r = await client.get("/api/v1/observations?offset=0&limit=-5")
    ...
    async def test_list_with_zero_limit(self, client: AsyncClient) -> None:
        ...
        r = await client.get("/api/v1/observations?offset=0&limit=0")
        assert r.status_code in [422, 400, 200]  # Depends on validation
...
class TestRequestBodyValidation:
    ...
    async def test_create_with_extra_fields(self, client: AsyncClient) -> None:
        ...
        payload = {**OBSERVATION_API, "extra_field": "should_be_ignored"}
    ...
    async def test_create_with_oversized_tags(self, client: AsyncClient) -> None:
        ...
        payload = {**OBSERVATION_API, "tags": [f"tag-{i}" for i in range(100)]}
        r = await client.post("/api/v1/observations", json=payload)
        assert r.status_code == 422

    async def test_create_with_non_dict_data(self, client: AsyncClient) -> None:
        """data field must be a dict."""
        payload = {**OBSERVATION_API, "data": "not-a-dict"}
        r = await client.post("/api/v1/observations", json=payload)
        assert r.status_code == 422
...
class TestObservationFixtures:
    """Verify observation fixtures work correctly."""

    async def test_created_observation_has_id(self, created_observation: dict) -> None:
        """created_observation fixture has id field."""
        assert "id" in created_observation
        assert isinstance(created_observation["id"], int)

    async def test_created_observation_has_source(
        self, created_observation: dict
    ) -> None:
        """created_observation fixture has source field."""
        assert "source" in created_observation
        assert isinstance(created_observation["source"], str)

    async def test_created_observations_returns_multiple(
        self, created_observations: list[dict]
    ) -> None:
        """created_observations fixture returns 3 observations."""
        assert len(created_observations) == 3
        for observation in created_observations:
            assert "id" in observation
            assert "source" in observation

    async def test_observation_payload_is_mutable(
        self, observation_payload: dict
    ) -> None:
        """observation_payload fixture returns mutable copy."""
        original_source = observation_payload["source"]
...
class TestErrorResponses:
    ...
    async def test_422_has_detail(self, client: AsyncClient) -> None:
        ...
        r = await client.post("/api/v1/observations", json={"data": {}})
        assert r.status_code == 422
        body = r.json()
        assert "detail" in body
