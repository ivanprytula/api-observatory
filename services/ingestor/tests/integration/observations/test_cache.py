...
async def test_cache_miss_populates_cache(client_with_cache: AsyncClient) -> None:
    ...
    create_resp = await client_with_cache.post(
        "/api/v1/observations", json=OBSERVATION_API
    )
    assert create_resp.status_code == 201
    observation_id = create_resp.json()["id"]

    # GET should miss cache, fetch from DB, and populate cache
    get_resp = await client_with_cache.get(f"/api/v1/observations/{observation_id}")
    assert get_resp.status_code == 200
    data = get_resp.json()
    assert data["source"] == OBSERVATION_API["source"]
...
async def test_cache_hit_skips_db(client_with_cache: AsyncClient) -> None:
    ...
    first_get = await client_with_cache.get(f"/api/v1/observations/{observation_id}")
    assert first_get.status_code == 200

    # Second GET: should hit cache
    # (In a real test with DB spying, we'd verify no additional DB query)
    second_get = await client_with_cache.get(f"/api/v1/observations/{observation_id}")
...
async def test_delete_invalidates_cache(client_with_cache: AsyncClient) -> None:
    ...
    # Populate cache
    get_resp1 = await client_with_cache.get(f"/api/v1/observations/{observation_id}")
    assert get_resp1.status_code == 200

    # Delete the observation (should invalidate cache)
    delete_resp = await client_with_cache.delete(
        f"/api/v1/observations/{observation_id}"
    )
    assert delete_resp.status_code == 204

    # GET should return 404 (cache invalidated, DB returns None)
    get_resp2 = await client_with_cache.get(f"/api/v1/observations/{observation_id}")
...
async def test_patch_process_invalidates_cache(client_with_cache: AsyncClient) -> None:
    ...
    process_resp = await client_with_cache.patch(
        f"/api/v1/observations/{observation_id}/process"
    )
