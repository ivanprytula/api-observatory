...
class TestNPlusOneCRUD:
    ...
    async def test_get_observations_with_tag_counts_naive_returns_correct_shape(
        self, db: AsyncSession, sample_observations_with_tags: list[Observation]
    ) -> None:
        """Naive function returns correct data shape."""
        if not sample_observations_with_tags:
            pytest.skip("No sample observations with tags created")

        result = await get_observations_with_tag_counts_naive(db, limit=5)
    ...
    async def test_get_observations_with_tag_counts_optimized_returns_correct_shape(
        self, db: AsyncSession, sample_observations_with_tags: list[Observation]
    ) -> None:
        ...
        result = await get_observations_with_tag_counts(db, limit=5)
    ...
    async def test_naive_and_optimized_produce_identical_tag_counts(
        ...
        naive_results = await get_observations_with_tag_counts_naive(db, limit=10)
        optimized_results = await get_observations_with_tag_counts(db, limit=10)
    ...
    async def test_limit_parameter_respected(
        ...
        for limit in [1, 3, 5]:
            naive = await get_observations_with_tag_counts_naive(db, limit=limit)
            optimized = await get_observations_with_tag_counts(db, limit=limit)
    ...
    async def test_tag_count_zero_for_empty_tags(
        self, db: AsyncSession, observation_timestamp
    ) -> None:
        """Observations with empty tag lists return tag_count=0."""
        ...
        # Create a observation with empty tags
        ...
        # Query via naive
        naive_results = await get_observations_with_tag_counts_naive(db, limit=1)
        naive_match = [r for r in naive_results if r["id"] == observation.id]
        assert len(naive_match) == 1
        assert naive_match[0]["tag_count"] == 0

        # Query via optimized
        optimized_results = await get_observations_with_tag_counts(db, limit=1)
        opt_match = [r for r in optimized_results if r["id"] == observation.id]
    ...
    async def test_tag_count_matches_actual_tags(
        ...
        # Create a observation with 5 tags
    ...
    async def test_excludes_soft_deleted_observations(
        self, db: AsyncSession, sample_observations_with_tags: list[Observation]
    ) -> None:
        """Soft-deleted observations are excluded from both functions."""
        from services.ingestor.repositories.observations import soft_delete_observation

        if not sample_observations_with_tags:
            pytest.skip("No sample observations with tags created")

        observation_to_delete = sample_observations_with_tags[0]
        await soft_delete_observation(db, observation_to_delete.id)

        naive_results = await get_observations_with_tag_counts_naive(db, limit=100)
        optimized_results = await get_observations_with_tag_counts(db, limit=100)

        # Deleted observation should not appear
        naive_ids = [r["id"] for r in naive_results]
        optimized_ids = [r["id"] for r in optimized_results]

        assert observation_to_delete.id not in naive_ids
...
class TestNPlusOneEndpoint:
    ...
    async def test_endpoint_returns_timing_data(
        self, client: AsyncClient, sample_observations_with_tags: list[Observation]
    ) -> None:
        """Endpoint returns timing comparison JSON."""
        if not sample_observations_with_tags:
            pytest.skip("No sample observations with tags created")

        response = await client.get("/api/v2/observations/n-plus-one-demo?limit=5")
        ...
        assert isinstance(data["observations_count"], int)
    ...
    async def test_endpoint_limit_parameter(
        ...
        for limit in [1, 5, 10]:
            response = await client.get(
                f"/api/v2/observations/n-plus-one-demo?limit={limit}"
            )
            assert response.status_code == 200
            data = response.json()
            assert data["limit"] == limit
            assert data["observations_count"] <= limit
    ...
    async def test_endpoint_default_limit(
        ...
        response = await client.get("/api/v2/observations/n-plus-one-demo")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_endpoint_validates_limit(self, client: AsyncClient) -> None:
        ...
        response = await client.get("/api/v2/observations/n-plus-one-demo?limit=0")
        assert response.status_code == 422  # Validation error

        # Test limit too large
        response = await client.get("/api/v2/observations/n-plus-one-demo?limit=101")
        assert response.status_code == 422  # Validation error

        # Test within bounds
        response = await client.get("/api/v2/observations/n-plus-one-demo?limit=50")
        assert response.status_code == 200
    ...
    async def test_speedup_naive_typically_slower_than_optimized(
        ...
        Note: With 0-1 observations, speeds may be similar due to DB overhead.
        With 10+ observations, naive typically shows 1.5x-3x+ slower execution.
        """
        if not sample_observations_with_tags:
            pytest.skip("No sample observations with tags created")

        response = await client.get("/api/v2/observations/n-plus-one-demo?limit=10")
        assert response.status_code == 200
        data = response.json()
